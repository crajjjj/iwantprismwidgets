...
; ----------------------------------------------------------------------------
; Prisma Edition fork of iWant Status Bars 2.09 (DaemonPrime, MIT). Same
; public API, plus two accessors dependent mods use to mirror bar visibility
; and autohide state (_getBarVisible, _getBarLastChangeTime). "Prisma Edition
...
; Prisma Edition patch: per-bar timestamp of last icon status change. Used by
...
; Prisma Edition patch: real-time timestamp of the bar's most recent icon
...
{Sets the active status to be used by the status bar}
	...
	If bar >= 0
		...
		EndIf
		; Prisma Edition patch: record the time of the most recent icon change
